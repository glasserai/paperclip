...
On Cloud, the new-agent picker and direct setup links allow Claude, Codex, OpenCode, and Grok. Grok uses the existing `grok_local` adapter with an xAI subscription or API-key connection and the managed sandbox environment. An adapter must also be loaded and enabled to be available.
